...
import com.bcyportfolio.shopping.mappers.IUserMapper;
...
public class UserService {
    ...
    public void sendCode(ForgotEmailSendCodeVo forgotEmailSendCodeVo){
        ...
        forgotEmailSendCodeVo.setAuthCode(code);
    ...
    public void sendCode(ForgotPasswordSendCodeVo vo) throws MessagingException {
        ...
        mimeMessageHelper.setSubject("[사이트] 비밀번호 재설정 요청");
        mimeMessageHelper.setText(String.format("%s<br>%s<br>%s%<br>%s<br>",
                "<h1>비밀번호 재설정</h1>",
                "<h2>[BCY SITE] 회원님의 비밀번호 재설정 요청이 있어 인증코드를 보냅니다. 인증번호를 입력해주세요</h2>",
                "<strong>인증번호 : " + code + "</strong>",
                "<b>비밀번호 재설정을 요청한적이 없는 경우 해당 메일을 폐기하세요.</b>"), true);
        mimeMessageHelper.setFrom("bcy2324@gmail.com");
...
//        System.out.println("resetPassword");
    ...
    public void withdrawal(LoginVo loginVo) {
        this.userMapper.deleteUser(loginVo.getEmail());
    }
